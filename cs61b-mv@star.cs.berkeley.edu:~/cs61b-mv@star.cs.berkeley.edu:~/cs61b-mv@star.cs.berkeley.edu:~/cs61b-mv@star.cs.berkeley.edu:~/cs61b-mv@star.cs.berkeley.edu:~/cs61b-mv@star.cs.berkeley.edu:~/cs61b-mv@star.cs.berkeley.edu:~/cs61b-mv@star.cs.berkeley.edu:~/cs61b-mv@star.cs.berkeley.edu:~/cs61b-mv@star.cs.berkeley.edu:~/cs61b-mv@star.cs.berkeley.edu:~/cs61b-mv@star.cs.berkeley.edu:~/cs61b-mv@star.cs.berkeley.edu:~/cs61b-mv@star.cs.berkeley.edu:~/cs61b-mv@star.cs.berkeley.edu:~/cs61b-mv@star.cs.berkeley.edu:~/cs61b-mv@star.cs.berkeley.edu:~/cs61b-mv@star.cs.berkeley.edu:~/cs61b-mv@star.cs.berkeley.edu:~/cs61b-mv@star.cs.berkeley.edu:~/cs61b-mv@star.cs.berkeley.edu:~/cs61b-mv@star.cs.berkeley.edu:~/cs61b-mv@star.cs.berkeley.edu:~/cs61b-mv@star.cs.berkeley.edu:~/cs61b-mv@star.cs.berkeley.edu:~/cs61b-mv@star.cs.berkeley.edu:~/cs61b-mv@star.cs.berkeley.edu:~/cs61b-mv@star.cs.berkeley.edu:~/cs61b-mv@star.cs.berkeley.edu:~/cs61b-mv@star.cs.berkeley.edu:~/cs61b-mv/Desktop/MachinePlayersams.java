...
public class MachinePlayer extends Player {
	...
	if ((m.x1==0 && m.y1==0) || (m.x1==0 && m.y1==7) || 
	    (m.x1==7 && m.y1==0) || (m.x1==7 && m.y1==7)){ //false for corners
	    return false;
	} else if (side == 1 && (m.y1==0 || m.y1==7)) { // false for opponent's goals
	    return false;
	} else if (side == 0 && (m.x1==0 || m.x1==7)) { // false for opponent's goals
	    return false;
	} else if (m.moveKind == 1) {  // add move
	    int x1 = m.x1;
	    int y1 = m.y1;
