...
public class MachinePlayer extends Player {
	...
	public Move chooseMove() {
		...
		if (game.getCount(side) < 2) {
			...
			Move chosenMove =  miniMax(side, -1, 1, game, 1, new Move()).bestMove; 	///1 should be depth
			System.out.println("Minimax returns valid move?"+isValidMove(game,chosenMove,side));
			if (isValidMove(game, chosenMove, side)) {
				return chosenMove;
			} else {
				return new Move();
			}
	...
	public Best miniMax(int side, double a, double b, GameBoard g, int depth, Move newMove) {
		...
		if(g.hasValidNetwork(this.side)){  //base cases
			...
			return new Best(newMove, 0); //evaluation(g) to test eval, 0 to short circuit eval and test minimax
		}
	...
	public double evaluation(GameBoard certainGame) {
		if (certainGame.hasValidNetwork(side)) { //if you win
			return 1;
		} else if (certainGame.hasValidNetwork(otherside)) { //if opponent wins
			return -1;
		}
		int yourPairs = 0;
		int opponentPairs = 0;
		...
		if (evalHash.find(certainGame) != null) { //gameboard is in hashtable
			...
			System.out.println("Did not find gameboard in hashtable");
			for (int i = 0; i < 8; i ++) {
				for (int j = 0; i < 2; j++) { //loop through the whole gameboard
					//System.out.println("HasChip? " + game.hasChip(i, j));
					if (game.hasChip(i, j)) { //if you find a chip...
                    	System.out.println("side Has " + game.getCount(side) + "chips");	
						System.out.println("sideHasChip? " + game.sideHasChip(i, j, side));
						if (game.sideHasChip(i, j, side)) { //your side (getChip(i, j).getSide() == this.side)
							System.out.println("counting your pairs");
							int countYourPairs = game.getChip(i, j).getConnections(game).length();
							yourPairs += countYourPairs;
							System.out.println("countYourPairs equals " + countYourPairs);
						} else if (game.getChip(i, j).getSide() == otherside) { //opponent side
						        	System.out.println("counting opponent pairs");
									System.out.println("otherside Has " + game.getCount(otherside) + "chips");
									System.out.println(game.getChip(i, j).getConnections(game).length() + "length of getconnections");	
									int countOppPairs = game.getChip(i, j).getConnections(game).length();
		...
		System.out.println("counted pairs");
